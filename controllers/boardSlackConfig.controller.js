...
class BoardSlackConfigController {
  async getBoardConfig(req, res) {
    try {
      ...
      const config = await boardSlackConfigService.getOrCreateConfig(board_id);
      
      // Ẩn webhook URL đầy đủ để bảo mật
      const safeConfig = {
  ...
  async updateBoardConfig(req, res) {
    try {
      ...
      const updatedConfig = await boardSlackConfigService.updateConfig(board_id, userId, updateData);
...
: null,
      ...
      res.json({
        success: true,
        message: 'Cập nhật cấu hình Slack cho board thành công',
        data: safeConfig
      });
    } catch (error) {
      res.status(400).json({
  ...
  async toggleNotifications(req, res) {
    try {
      ...
      const config = await boardSlackConfigService.toggleNotifications(board_id, userId, is_active);

      res.json({
        success: true,
        message: `Thông báo Slack của board đã ${is_active ? 'bật' : 'tắt'}`,
        data: {
  ...
  async testWebhook(req, res) {
    try {
      ...
      const testMessage = `🧪 Test thông báo từ KEN Board\n\nBoard ID: ${board_id}\n\nNếu bạn thấy tin nhắn này, webhook URL của board đã hoạt động!`;
      ...
      try {
        ...
        if (testResult) {
          res.json({
            success: true,
            message: 'Webhook URL hoạt động! Vui lòng kiểm tra Slack channel của board.'
          });
        } else {
          res.status(400).json({
            success: false,
            message: 'Không thể gửi thông báo. Vui lòng kiểm tra lại webhook URL và đảm bảo webhook vẫn còn hoạt động trong Slack.'
          });
        }
      } catch (slackError) {
        // Nếu slackService throw error, trả về message chi tiết
        res.status(400).json({
          success: false,
message: slackError.message || 'Không thể gửi thông báo đến Slack. Vui lòng kiểm tra lại webhook URL.'
        });
      }
    } catch (error) {
      res.status(400).json({
        success: false,
        message: error.message || 'Có lỗi xảy ra khi test webhook. Vui lòng thử lại sau.'
      });
    }
  }
  async deleteBoardConfig(req, res) {
    try {
      const { board_id } = req.params;
      const userId = req.user?.id;
      if (!userId) {
      ...
      await boardSlackConfigService.deleteConfig(board_id, userId);

      res.json({
        success: true,
        message: 'Đã xóa cấu hình Slack của board'
      });
    } catch (error) {
      res.status(400).json({
        success: false,
...
module.exports = new BoardSlackConfigController();
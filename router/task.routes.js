...
router.use((req, res, next) => {
  if (req.method === 'PUT' && req.path.includes('toggle-star')) {
  }
  next();
});
...
router.get('/:taskId/attachments', authenticateAny, taskController.getAttachments);

router.delete('/:taskId/attachments', authenticateAny, taskController.deleteAttachment);

// ==================== USER RELATED ROUTES (phải đặt trước /:id) ====================
...
router.get('/my/starred', authenticateAny, taskController.getStarredTasks);
...
// ==================== TASK SPECIFIC ROUTES (phải đặt trước /:id) ====================
...
router.put(
  ...
  authenticateAny,
  (req, res, next) => {
    // Nếu URL chứa "toggle-star", đây là lỗi routing - route generic đang match trước
    if (req.url.includes('toggle-star') || req.originalUrl.includes('toggle-star')) {
